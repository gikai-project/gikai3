...
def check_api_limit():
    if st.session_state.api_calls >= MAX_CALLS:
        st.error(
            f"⚠ API利用上限に達しました（{MAX_CALLS}回）。これ以上は実行できません。"
        )
        st.stop()
...
# ======================================================
SCORE_EXPLANATION = {
    5: "完全充足。具体・一義的で第三者が同一理解に到達。実務で修正不要。",
    4: "実務上ほぼ問題なし。一部に軽微な抽象や補足不足あり。",
    3: "最低限達成。触れてはいるが抽象的で、追加説明が必要。",
    2: "不足が明確。観点の一部にしか触れておらず実務に結びつかない。",
    1: "形式的・断片的。キーワードが表面的に出てくるのみ。",
    0: "未達・評価不能。当該観点に全く触れていない。"
}
